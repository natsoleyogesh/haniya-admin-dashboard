...
const EditCategoryPage: React.FC<EditCategoryPageProps> = ({ setActivePage }) => {
    const { editingCategory, updateCategory, setEditingCategory } = useData();
    ...
    if (!editingCategory) {
        return <div className="text-center p-4">Loading or no category selected...</div>;
    }

    return (
        <div className="p-8 max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md">
            <h1 className="text-2xl font-semibold text-gray-800 dark:text-white mb-6">Edit Category</h1>
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-6">
                    <label htmlFor="categoryStatus" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Status</label>
                    <select
                        id="categoryStatus"
                        value={status}
